...
class Shipping extends Template
{
    ...
    const XML_PATH_STORE_TOTAL_TIMESLOT_FORMAT = 'magedelight_ScheduleShipping/timeslot/hours_format';

    const XML_PATH_STORE_TOTAL_TIMESLOT_12 = 'magedelight_ScheduleShipping/timeslot/add_timeslot_12';

    const XML_PATH_STORE_TOTAL_TIMESLOT_24 = 'magedelight_ScheduleShipping/timeslot/add_timeslot_24';
    ...
    public function isScheduleShippingEnable()
    {
        ...
        return $this->scopeConfig->getValue(self::XML_PATH_STORE_ENABLE_EXTENSION, $storeScope);
    }
    ...
    public function isScheduleShippingEnableForCustomer()
    {
        ...
        if (in_array($customerGroupId, $customer_groups_array, true)) {
            return true;
        }
     ...
     * @return int
     */
    public function getTimeslotIntervalHour(): int
    {
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
    ...
    public function getTimeSlot()
    {
         ...
         * @ TimeSlot
         */
        $_totalTimeSlot = $this->scopeConfig->getValue(self::XML_PATH_STORE_TOTAL_TIMESLOT_12, $storeScope);
        if ($this->scopeConfig->getValue(self::XML_PATH_STORE_TOTAL_TIMESLOT_FORMAT, $storeScope)) {
            $_totalTimeSlot = $this->scopeConfig->getValue(self::XML_PATH_STORE_TOTAL_TIMESLOT_24, $storeScope);
        }
        ...
        $priceHelper = $objectManager->create(Data::class);
        ...
        for ($i = 0; $i < $_showDeliveryDay; $i++) {
            ...
            foreach ($_totalTimeSlots as $_totalTimeSlot) {
                ...
                $_slot['label'] = $_totalTimeSlot['start_time'] . '-' . $_totalTimeSlot['end_time'];
                if ($_totalTimeSlot['timeslot_price'] > 0) {
    ...
    {
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $start_time = $this->scopeConfig->getValue(self::XML_PATH_STORE_TOTAL_TIMESLOT_12, $storeScope);
        if ($this->scopeConfig->getValue(self::XML_PATH_STORE_TOTAL_TIMESLOT_FORMAT, $storeScope)) {
            $start_time = $this->scopeConfig->getValue(self::XML_PATH_STORE_TOTAL_TIMESLOT_24, $storeScope);
        }
